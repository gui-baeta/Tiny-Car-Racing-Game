...
MyMesh createISTlogo() {
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(ist_vertices), ist_vertices, GL_STATIC_DRAW);


	glEnableVertexAttribArray(NORMAL_ATTRIB);
	glVertexAttribPointer(NORMAL_ATTRIB, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), 0);
	glEnableVertexAttribArray(VERTEX_COORD_ATTRIB);
	glVertexAttribPointer(VERTEX_COORD_ATTRIB, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));

	// unbind the VAO
	glBindVertexArray(0);

	amesh.numIndexes = sizeof(ist_vertices) / (6 * sizeof(float));
	amesh.type = GL_TRIANGLES;
	return(amesh);
}
